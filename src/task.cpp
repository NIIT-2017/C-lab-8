...
int IfEmpty(int sum)	//если файл пуст,то выводим сообщение об этом
{
	if (sum == 0)
	{
...
void kompressor(char ch, int number, char* name, char* extension)
{
	...
	int count = 1;
	fwrite("key", 1, 3, fp_compressed); // сигнатура (уникальная подпись) 
	fwrite(&count, 4, 1, fp_compressed); // количество уникальных символов
	fwrite(&ch, 1, 1, fp_compressed); // символ
	fwrite(&number, 4, 1, fp_compressed); // частота повторяющихся символов
	for (int i = 0; i < 8; i++)
		fwrite(&extension[i], 1, 1, fp_compressed); // расширение исходного файла
...
struct SYM* buildTree(struct SYM* psyms[], int count)
{
	...
	// Записываем сумму частот последнего и предпоследнего элементов массива psyms в поле частоты временного узла. 
	temp->freq = psyms[count - 2]->freq + psyms[count - 1]->freq;
	// Записать сумму частот последнего и предпоследнего элементов массива psyms в поле частоты временного узла.
	temp->left = psyms[count - 1];
	temp->right = psyms[count - 2];
	temp->code[0] = 0;
	if (count == 2) // сформирован корневой элемент с частотой 1.0
		return temp;
	// добавление temp в psyms сортировкой
	int position = count - 3;
	//последние позиции с использованными частотами
...
{
	fwrite("key", 1, 3, fp_compressed); // сигнатура (уникальная подпись)
	fwrite(count, 4, 1, fp_compressed); // частота уникальных символов
	for (int i = 0; i < *count; i++)
	{
...
{
	int temp = 0;
	FILE* fp_codes = fopen("101bycom.txt", "rb"); // 101 файл
	if (!fp_codes)
	{
		perror("File:");
		exit(1);
...
void unkompressor(FILE* fp_compressed, char* name)
{
	int ch = 0;
	int number = 0;
	fread(&ch, 1, 1, fp_compressed); // символ
	fread(&number, 4, 1, fp_compressed); // частота
	char extension[8] = { 0 };// 
	for (int i = 0; i < 8; i++)
	{
...
{
	FILE* fp_101 = fopen("101bydecom.txt", "wb"); // 101-файл
	if (!fp_101)
	{
		perror("File:");
		exit(1);
